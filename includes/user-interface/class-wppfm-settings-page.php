...
if ( ! class_exists( 'WPPFM_Settings_Page' ) ) :
	...
	class WPPFM_Settings_Page {
		...
		private function settings_content() {
			...
			$html .= '<tr vertical-align="top" class="wppfm-setting-selector">';
			...
			if ( 'full' === WPPFM_PLUGIN_VERSION_ID ) {
				$html .= '<tr vertical-align="top" class="wppfm-setting-selector">';
				$html .= '<th scope="row" class="titledesc">' . esc_html__( 'Manual channel update', 'wp-product-feed-manager' ) . '</th>';
				$html .= '<td class="forminp forminp-checkbox">';
				$html .= '<fieldset>';
				$html .= '<input name="wppfm-manual-channel-update" id="wppfm-manual-channel-update" type="checkbox" class="" value="1"' . $manual_channel_update . '> ';
				$html .= '<label for="wppfm-manual-channel-update">';
				$html .= esc_html__( 'When switched on, you need to manually update channels (default off).', 'wp-product-feed-manager' ) . '</label>';
				$html .= '<p><i>' . esc_html__( 'By default your channels are updated automatically as soon as an update is available. This option allows your to switch off the automatic channel updates and keep your channels manually up to date.', 'wp-product-feed-manager' ) . '</i></p></fieldset>';
				$html .= '</td></tr>';
			}
